module Jqgrid
    ...
    def jqgrid(title, id, action, columns = [], options = {})
      ...
      options =
        {
          ...
          # the currently loaded values in the form.
      ...
      if options[:onclick_pg_buttons] == 'null'
        # If no error handlers return true
        options[:onclick_pg_buttons] = 'true;'
      end
      ...
      if options[:search] == 'true'
        search = %Q/.navButtonAdd("##{id}_pager",{caption:"",title:'Find Records', buttonicon : '#{get_search_icon(options)}', onClickButton:function(){ mygrid[0].toggleToolbar() } })/
      ...
      if options[:subgrid].present?
        ...
        # onclickPgButtons handlers
           ...
           # If no handlers return true
           options[:subgrid][:onclick_pg_buttons] = 'true;'
         end
        ...
        subGridRowExpanded: function(subgrid_id, row_id) {
            ...
            .navButtonAdd("#"+pager_id,{caption:"",title:'Find Records', buttonicon :'#{get_search_icon(options)}', onClickButton:function(){ subgrd[0].toggleToolbar() } })
    ...
    def get_sub_options(editoptions)
      ...
      editoptions.each do |couple|
        if couple[0] == :value # :value => [[1, "Rails"], [2, "Ruby"], [3, "jQuery"]]
          ...
          if couple[0] == :elmsuffix || couple[1].instance_of?(Integer) || couple[1] == 'true' || couple[1] == 'false' || couple[1] == true || couple[1] == false || (couple[1].is_a?(String) && couple[1].match?(/function/))
              options << %Q/#{couple[0]}:#{couple[1]},/
            else
              options << %Q/#{couple[0]}:"#{couple[1]}",/          
          end
